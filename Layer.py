...
class Layer:
    ...
    def transformer_layer(self,x, num_heads, d_model, d_ff):
    ...
    def backward_through_decoder(self, d_output, encoder_outputs):
        # Градиенты для self-attention
        d_self_attn = self.backward_attention(self.self_attn, d_output)
        # Градиенты для cross-attention
        d_cross_attn = self.backward_attention(self.cross_attn, d_output, encoder_outputs)
        # Градиенты для feed-forward
        d_ffn = self.backward_feed_forward(self.feed_forward, d_output)
        return d_ffn

    def backward_through_encoder(self, d_output, src):
        # Градиенты для self-attention
        d_self_attn = self.backward_attention(self.self_attn, d_output)
        # Градиенты для feed-forward
        d_ffn = self.backward_feed_forward(self.feed_forward, d_output)
